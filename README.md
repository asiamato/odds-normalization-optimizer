...
$$\Delta^{n-1} = \left\{ x \in \mathbb{R}^n \mid \sum_{i=1}^n x_i = 1, x_i \ge 0 \right\}$$
...
* **Simplex Projection (Our Approach):** We formulate coherence restoration as a strictly convex optimization problem. We seek the vector $P_{proj} \in \Delta^{n-1}$ that minimizes the Euclidean distance (L2-norm) from the original $P_{raw}$:
  ...
  $$\mathcal{L}(x, \lambda, \mu) = \frac{1}{2} ||x - P_{raw}||^2_2 - \lambda \left( \sum_{i=1}^n x_i - 1 \right) - \sum_{i=1}^n \mu_i x_i$$
...
* **Generalized Kullback-Leibler (KL) Divergence:** Since $P_{raw}$ does not sum to 1, standard relative entropy is undefined. We employ the Generalized KL Divergence to measure how much original signal is destroyed during normalization:
  ...
  $$D_{KL}(P || Q) = \sum_{i=1}^n \left( P_i \ln\left(\frac{P_i}{Q_i}\right) - P_i + Q_i \right)$$
...
## Application Note
This repository highlights a research-driven approach to Software Engineering. It was developed as a core portfolio project for the **UIUC++ Summer Research Program**, demonstrating proficiency in convex optimization, information theory, scoring axioms, and polyglot software design.